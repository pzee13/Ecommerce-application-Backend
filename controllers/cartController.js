...
const getCart = async (req, res) => {
  try {
    ...
    const cart = await Cart.findOne({ user: userId }).populate({
      ...
      // Ensure that each item has a valid product
      const items = cart.items.map(item => ({
        product: item.product || {}, // Ensure product is defined
        quantity: item.quantity,
      }));
    ...
    res.json({
      ...
      items: cart.items.map(item => ({
        product: {
          ...
          name: item.product.name,
          price: item.product.price,
          stock: item.product.stock, // Include stock information
        },
        quantity: item.quantity,
      })),
      totalAmount,
...
const updateCart = async (req, res) => {
  ...
  try {
    ...
    const cart = await Cart.findOne({ user: userId }).populate({
      path: 'items.product',
      select: 'name price stock', // Populate necessary product details
    });
